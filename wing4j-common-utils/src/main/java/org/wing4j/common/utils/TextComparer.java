...
public class TextComparer {
	...
	private static List<TextDiff> calcDiffs(Point startPoint, boolean[][] barr,
		...
		if (startPoint.getX() != 0) {
			diffs
					.add(new TextDiff(TextDiff.TYPE_INSERTED, 0, startPoint
							.getX()));
		}
		if (startPoint.getY() != 0) {
			diffs
					.add(new TextDiff(TextDiff.TYPE_DELETED, 0, startPoint
							.getY()));
		}
	...
	public static void main(String[] args) {
		...
		// TextComparer.compare("YBCACADF", "ZCXCADFESBABCACA");
		 List<TextDiff> diffs1 = TextComparer.compare("a文本怎么比较bf", "文本如何比较");
		 System.out.println(diffs1.toString());
